...
ALTER TABLE jugadores DROP CONSTRAINT IF EXISTS jugadores_posicion_check;

-- Add new constraint with all valid positions
ALTER TABLE jugadores ADD CONSTRAINT jugadores_posicion_check 
  CHECK (posicion IN ('Forward', 'Back', 'Pilar', 'Hooker', 'Segunda', 'Ala', 'N8', 'Medio', 'Apertura', 'Centro', 'Wing', 'Fullback'));
...
-- Update existing records with sample point data
...
-- Delete existing sample data and insert fresh players
DELETE FROM jugadores;

-- Insert players with specific rugby positions
INSERT INTO jugadores (nombre, posicion, precio, puntos_totales, tendencia, foto_url) VALUES
